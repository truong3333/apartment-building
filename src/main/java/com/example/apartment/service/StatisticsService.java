...
public class StatisticsService {
    ...
    public StatisticsResponse getStatistics(StatisticsRequest request){
        ...
        return StatisticsResponse.builder()
                ...
                .apartmentUsageRate(Math.round(((double) roomUsage.size()/apartmentRepository.count() * 10000.0) /100.0))
                .userInSize(userIn.size())
                .userOutSize(userOut.size())
                .listUserIn(listUserIn)
                .listUserOut(listUserOut)
                .totalAmount(Math.round(monthlyCostRepository.sumTotalAmountByMonthAndYear(request.getMonth(), request.getYear())))
